...
# Read metadata file - check for environment variable first (from run_metadata_merge.R)
# Otherwise auto-detect file with "metadata" in the name
metadata_file <- Sys.getenv("METADATA_FILE", unset = "")
if (metadata_file == "") {
  # Auto-detect metadata file in input_files/
  metadata_files <- list.files("input_files", pattern = "metadata", ignore.case = TRUE, full.names = TRUE)
  if (length(metadata_files) == 0) {
    stop("No metadata file found in input_files/. Expected a file with 'metadata' in the name.")
  }
  if (length(metadata_files) > 1) {
    # Use most recent
    metadata_file <- metadata_files[which.max(file.mtime(metadata_files))]
    cat("Multiple metadata files found, using:", basename(metadata_file), "\n")
  } else {
    metadata_file <- metadata_files[1]
  }
}
cat("Using metadata file:", basename(metadata_file), "\n")
...
biomarkers_postQC <- read_csv("input_files/NPQ_post_QC.csv")
biomarkers_low_postQC <- read_csv("input_files/NPQ_Low_post_QC.csv")
...
## Samples in NPQ Files Not in Metadata
...
# Get all unique sample IDs from combined biomarkers
all_npq_samples <- combined_biomarkers %>%
  select(SampleID) %>%
  distinct()

# Find samples not in metadata (excluding technical replicates like SC, NC, IPC, HIHG)
# Technical replicates typically have patterns like "SC", "NC", "IPC", or contain "HIHG"
...
samples_not_in_metadata <- all_npq_samples %>%
  ...
  mutate(
    is_technical_rep = grepl(technical_rep_pattern, SampleID, ignore.case = TRUE)
  )

# Separate technical replicates from other unmatched samples
...
cat("=== SAMPLES IN NPQ FILES NOT IN METADATA ===\n\n")

if (nrow(unmatched_samples) > 0) {
  cat("**WARNING:** Found", nrow(unmatched_samples), "sample(s) in NPQ files that are NOT in metadata:\n\n")
  for (s in unmatched_samples$SampleID) {
    cat("  -", s, "\n")
  }
  cat("\nThese samples will NOT be included in merged output files.\n")
  cat("Please verify if these samples should be added to the metadata file.\n\n")

  # Write to file for reference
  write_csv(unmatched_samples %>% select(SampleID),
            "output_files/samples_not_in_metadata.csv")
  cat("List saved to: output_files/samples_not_in_metadata.csv\n\n")
} else {
  cat("All non-technical samples in NPQ files are present in metadata. ✓\n\n")
}

if (nrow(technical_reps) > 0) {
  cat("**Technical replicates excluded (expected):**", nrow(technical_reps), "samples\n")
  cat("  (These are QC samples like SC, NC, IPC, HIHG that are not expected in metadata)\n")
}
```
...
# NOTE: These are flagged for review but NOT automatically excluded
...
if (nrow(illogical_race_eth) > 0) {
  cat("**Illogical Race/Ethnicity Combinations (flagged for review, NOT auto-excluded):**\n")
  ...
  cat("  - These are flagged in exclude_reason but NOT automatically excluded\n")
  cat("  - Review in sample_exclusion_report.csv and manually set exclude=TRUE if needed\n\n")
} else {
