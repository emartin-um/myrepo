...
**OR**
1. Meta_plus_file <- NULL  # e.g., "meta_plus.csv" to skip data prep
2. Biomarker_groups_file <- NULL  # e.g., "biomarker_groups.csv" to skip clustering
...
if (is.null(Meta_plus_file)) {
  ...
  Meta_data <- "U19_Alamar_metadata_2025Dec17.csv" #SPECIFY THE FILE NAME CONTAINING META DATA
}
...
# Add this in the setup chunk after the other parameters
...
identify_outliers <- function(data, biomarker_groups, method = "iqr", threshold = 3, 
  ...
  for (biomarker in all_biomarkers) {
    ...
    if (length(outliers) > 0) {
      ...
      outlier_df <- outlier_df %>% arrange(desc(Value))
...
library(gridExtra)
library(emmeans)
library(car)  # Add to your libraries
