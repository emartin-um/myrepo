...
Input files are auto-detected from `input_files/`. If no NPQ file is found, the pipeline will attempt to copy from `Primary_QC/output_files/`.

**Required Input Data Files:** Place all files in a folder called `input_files` in the working directory (or let the pipeline copy them from Primary_QC).
...
# Check if input files exist, if not copy from Primary_QC
primary_qc_dir <- "../Primary_QC/output_files"
...
if (length(npq_files) == 0 && dir.exists(primary_qc_dir)) {
  # No NPQ file found - try to copy from Primary_QC
  primary_npq <- list.files(primary_qc_dir, pattern = "^NPQ.*\\.csv$", full.names = TRUE)
  # Exclude Low detectability files
  primary_npq <- primary_npq[!grepl("Low", primary_npq)]
  ...
  if (length(primary_npq) > 0) {
    ...
    message("Copied NPQ file from Primary_QC: ", basename(primary_npq))
    npq_files <- list.files("input_files", pattern = "^NPQ.*\\.csv$", full.names = TRUE)
  }
}
...
if (length(npq_files) > 0) {
  ...
  stop("No NPQ file found in input_files/ and could not copy from Primary_QC/output_files/")
}

# --- Raw counts file ---
if (!file.exists("input_files/Raw_counts.csv") && dir.exists(primary_qc_dir)) {
  primary_raw <- file.path(primary_qc_dir, "Raw_counts.csv")
  if (file.exists(primary_raw)) {
    file.copy(primary_raw, "input_files/")
    message("Copied Raw_counts.csv from Primary_QC")
  }
}

# --- Sample_QC.csv file ---
if (!file.exists("input_files/Sample_QC.csv") && dir.exists(primary_qc_dir)) {
  primary_qc <- file.path(primary_qc_dir, "Sample_QC.csv")
  if (file.exists(primary_qc)) {
    file.copy(primary_qc, "input_files/")
    message("Copied Sample_QC.csv from Primary_QC")
  }
}

# --- Replicate list ---
# This file must be created by analyst - cannot auto-copy
if (!file.exists("input_files/Replicate_list.csv")) {
  warning("Replicate_list.csv not found in input_files/. This file must be created manually.")
}
